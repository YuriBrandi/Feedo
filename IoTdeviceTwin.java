...
public class FeedoTwinDemo
{
            ...
            throws IOException, URISyntaxException
    {
        ...
        try
        {
            ...
            try{
                timeToFeed = Long.parseLong(twin.getDesiredProperties().get("TimeToFeed").toString());
                lastTimeFed = Long.parseLong(twin.getReportedProperties().get("LastTimeFed").toString());
            }
            catch (NumberFormatException e){
                System.out.println("Desired Property formatted incorrectly...");
            }

            //Check if timer has expired and if it has not already been satisfied.
            if(lastTimeFed == timeToFeed)
                System.out.println("Feeding timer already satisfied.");
            else
                if(timeToFeed <= currTime){
                    System.out.println("+++++++++FEEDING PET+++++++++++");
                    reportedProperties.put("LastTimeFed", timeToFeed);
                    System.out.println("Updating reported property \"LastTimeFed\" to value " + timeToFeed);
                    ReportedPropertiesUpdateResponse response = client.updateReportedProperties(reportedProperties);
                    System.out.println("Successfully set property \"LastTimeFed\" to value " + timeToFeed);
                    ...
                    System.out.println("Timer still not expired...");
